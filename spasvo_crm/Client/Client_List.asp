...
<script language="vbscript" type="text/vbscript">
	Function UserSQL()
		...
		if document.getElementById("Start_Update_Date").value <> "" then		
			Mysqlstr = Mysqlstr + "Client_Update_Date >= '" + document.getElementById("Start_Update_Date").value + "' and "		
		end if
		
		if document.getElementById("Over_Update_Date").value <> "" then		
			Mysqlstr = Mysqlstr + "Client_Update_Date <= '" + document.getElementById("Over_Update_Date").value + "' and "		
		end if		
		
		if document.getElementById("Client_Follow").value <> "XX" then		
			Mysqlstr = Mysqlstr + "Client_Follow='" + document.getElementById("Client_Follow").value + "' and "		
		end if
...
<%
	...
	End IF	
	'response.Write(Full_Client_sales)
...
<table width="100%" height="20" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#006699">
  <tr>
    <td bgcolor="#006699"><table width="80%" height="100%" border="0" align="center" cellpadding="0" cellspacing="0">
      <tr>
        <td><font color="#FFFFFF">----管理客户----</font></td>
      </tr>
    </table></td>
  </tr>
</table>
<br />
<form id="myform" name="myform" method="post" action="Client_list.asp">
  ...
  <tr>
    <td><table width="99%" border="0" align="center" cellpadding="0" cellspacing="1" style="margin:0 0 10px 0 ;">
      <tr>
        ...
        <td>背景</td>
        <td>
		<select name="Client_beijing" id="Client_beijing" style="width:124px;">
			  <%
      ...
      <tr>
        <td>状态</td>
        <td>
			<select name="Client_Follow" id="Client_Follow" style="width:124px;">
        ...
        <td>更新</td>
        <td><input name="Start_Update_Date" type="text" id="Start_Date" onfocus="setday(this)" /></td>
        <td>更新</td>
        <td><input name="Over_Update_Date" type="text" id="Start_Date2" onfocus="setday(this)" /></td>
        <td>&nbsp;</td>
        <td>&nbsp;</td>
      </tr>
    </table></td>
...
<table width="80%" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC">
  <tr align="center">
    ...
    <td bgcolor="#006699"><font color="#FFFFFF">次数</font></td>
    <td bgcolor="#006699"><font color="#FFFFFF">更新日期</font></td>
    <td bgcolor="#006699"><font color="#FFFFFF">创建日期</font></td>
...
%>   
  <tr align="center" bgcolor="<%=Mybgcolor%>">
    ...
    <td height="28" align="center" bgcolor="<%=Mybgcolor%>"><%IF trim(rs("Client_Follow"))="N" Then
	     response.Write("潜在客户") 
	...
	<%  Set RS1=dbconn.execute("Select count(*) as Follow_Client From T_Client_Follow Where Follow_Client='"& trim(rs("Client_Id")) &"'") %>
    <td height="28" align="center" bgcolor="<%=Mybgcolor%>"><%=trim(RS1("Follow_Client"))%></td>
    <td height="28" align="center" bgcolor="<%=Mybgcolor%>"><% IF  isnull(trim(rs("Client_Update_Date"))) then response.Write(trim(rs("Client_Date"))) else response.Write(trim(rs("Client_Update_Date"))) end if %></td>
    <td height="28" align="center" bgcolor="<%=Mybgcolor%>"><%=trim(rs("Client_Date"))%></td>
    <td width="40" height="28" align="center"><a href="Client_Edit.asp?Id=<%=trim(rs("Client_Id"))%>">编辑</a></td>
    <td width="40" align="center"><a href="client_list.asp?ID=<%=trim(rs("Client_Id"))%>&action=del" onClick="return confirm('是否要删除？');">删除</a></td>
    <td width="40" align="center"><a href="Client_View.asp?Id=<%=trim(rs("Client_Id"))%>">查看</a></td>
  </tr>
<%
	rs.movenext
	next
