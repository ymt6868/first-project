...
<script language="vbscript" type="text/vbscript">
	Function UserSQL()
		...
		if document.getElementById("Client_File_FileName").value <> "" then		
			Mysqlstr = Mysqlstr + "Client_File_FileName like'%" + document.getElementById("Client_File_FileName").value + "%' and "	
		end if
...
<%
	If Request("Action")="Select" and Request("FenYe")="" Then
		SQLstr=Request("SQLstr")&"1=1 order by Client_File_Date " & Request("Job_Order") & ""
		UserSql="1=1"
	ElseIf Request("Action")="Select" and Request("FenYe")="Rxby" Then
		...
		Page=Request("Page")
				...
				End If
	Else
		SQLstr="1=1 order by Client_File_Date desc"
				IF session("System_Key")="1" Then
					UserSql="1=1"
				Else
					Client_sales=session("User_Id")
...
<table width="100%" height="20" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#006699">
  <tr>
    <td bgcolor="#006699"><table width="80%" height="100%" border="0" align="center" cellpadding="0" cellspacing="0">
      <tr>
        <td><font color="#FFFFFF">----管理文档----</font></td>
      </tr>
    </table></td>
  </tr>
</table>
<br />
<form id="myform" name="myform" method="post" action="ClientFile_List.asp">
  ...
  <tr>
    <td><table width="99%" border="0" align="center" cellpadding="0" cellspacing="1" style="margin:0 0 10px 0 ;">
      <tr>
        ...
        <td>标题</td>
        <td><input name="Client_File_FileName" type="text" id="Client_File_FileName" /></td>
        <td>名称</td>
...
<table width="80%" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC">
  <tr align="center">
    ...
    <td bgcolor="#006699"><font color="#FFFFFF">客户</font></td>
    <td width="50" bgcolor="#006699"><font color="#FFFFFF">文档</font></td>
...
<%
	...
	Mysql="Select * From T_Client_File,T_Client,T_User Where Client_ID=Client_File_Client And Client_sales=User_Id And " & UserSql & " And "& SQLstr
...
%>   
  <tr align="center" bgcolor="<%=Mybgcolor%>">
    ...
    <td height="28" align="left" bgcolor="<%=Mybgcolor%>">&nbsp;<%=trim(rs("Client_File_FileName"))%></td>
    <td height="28" align="left" bgcolor="<%=Mybgcolor%>">&nbsp;<%=trim(rs("Client_Company"))%></td>
    <td height="28" bgcolor="<%=Mybgcolor%>"><a href="UploadFile/<%=trim(rs("Client_File_Url"))%>" target="_blank">查看</a></td>
    <td height="28" bgcolor="<%=Mybgcolor%>"><%=trim(rs("User_China"))%></td>
    <td height="28" align="center" bgcolor="<%=Mybgcolor%>"><%=trim(rs("Client_File_Date"))%></td>
    <td height="28" align="center" bgcolor="<%=Mybgcolor%>"><a href="ClientFile_List.asp?ID=<%=trim(rs("Client_File_ID"))%>&action=del" onClick="return confirm('是否要删除？');">删除</a></td>
    <td align="center" bgcolor="<%=Mybgcolor%>"><a href="ClientFile_View.asp?ID=<%=trim(rs("Client_File_ID"))%>">查看</a></td>
  </tr>
<%
	rs.movenext
	next
...
<%
	if request("action")="del" then
		...
		Set rs=dbconn.execute("Select * From T_Client_File Where Client_File_ID='"& Request("ID") &"'")
		
		'开始级联删除附件
		
			set fso = server.CreateObject("scripting.filesystemobject")			
			FileName=server.MapPath ("UploadFile/"& trim(rs("Client_File_Url")) &"")
		...
		'结束级联删除附件
		dbconn.execute("delete from T_Client_File where Client_File_ID="& request("ID") &"")
				
		response.write"<SCRIPT language=JavaScript>"
		response.write"this.location.href='ClientFile_List.asp';</SCRIPT>" 
	end if
%>
</body>
</html>
